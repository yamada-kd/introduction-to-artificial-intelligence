...
# Python は，科学技術界隈，特にデータ科学に関する分野において昨今，最も利用されているプログラミング言語です．これを習得することでプログラミング技術なしでは為し得なかったような大量データの解析ができるようになります．Python は C言語や Fortran のような伝統的なプログラミング言語と比べて簡単に学ぶことができるプログラミング言語です．ものすごく人気がある言語であり，たくさんのライブラリ（補助ツール）が存在しているので，色々なことができます．計算の速さもそこそこで，少なくとも悪くはありません．ここでは，Python のバージョン3（Python3）の使い方を学びます．

# ## Python の実行方法

# Python をどうやって実行するかとか，グーグルコラボラトリーでどうやって動かすかについて紹介します．

# ### 実行の手順

# 通常，Python は以下のような手順で使います．
# 
# 1.   テキストファイルにプログラムを書きます．プログラムの本体をソースコードと呼びます．
# 2.   書いたソースコードを保存します．拡張子は「.py」とします．
# 3.   そのプログラムを（コマンドライン等から）呼び出します．
# 
# この Google Colaboratory では以下の手順で Python を実行できます．
# 
# 1.   コードセルにプログラムを書きます．
# 2.   書いたプログラムを実行ボタンをクリックするか「Ctrl」と「Enter」を同時押しすることで実行します．
...
# 複数個の値を代入することができる変数があります．これのことをただの変数ではなく，Python ではリスト（list）と呼びます．リスト変数は `[` と `]` と `,` を使って生成します．以下のように生成した変数「list_a」の要素にアクセスするには「list_a[0]」のような感じでリスト変数に添字を付けて書きます．こうすることで「list_a」に格納されている最初の値である「10」にアクセスできます．多くのプログラミング言語ではリストの一番最初を示すインデックスは「0」です（1ではありません．）．こういうシステムのことはゼロオリジンと呼びます．
...
# 何らかの外部プログラムを実行したときのその出力結果を取得することがよくあります．以下のものは危険性を含む書き方なのであまり良くないのですが，以下のようにすると何かのプログラムを実行した結果を処理することができます．
...
# ```{note}
# インジェクションという単語で調べてみるとこのプログラムの危険性がわかります．危険がない状況だと利用しても問題はありません．
# ```
