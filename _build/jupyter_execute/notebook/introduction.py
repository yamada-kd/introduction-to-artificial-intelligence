...
# データ科学と機械学習の関連についてまとめてみます．

# ### データ科学

# データ科学という用語が意味するものはとても曖昧で漠然としています．データ科学は第 4 の科学とかって言われていますがこれまでの科学（理論科学，実験科学，計算機科学）でもデータ使ってましたよね？
# 
# 一方で，半導体技術の向上やインターネットの繁栄により大量のデータを扱えるようになると共にデータ科学が流行を迎えたという背景を考慮すると，従来の科学とデータサイエンスの差異は扱うデータの量にあるという側面はありそうではあります．
# 
...
# 機械学習の分野では日々新しい言葉が作られていたり，様々な分野から機械学習の研究をしようとしている人がいたりするため，用語の用法にコンセンサスが取れていない場合があります．そこでこの節では機械学習に関する用語の説明をします．
...
# 後の紹介する教師あり機械学習法で成長させる人工知能を利用して解決しようとする問題は大きく分けてふたつあります．ひとつは回帰問題です．回帰問題は人工知能に何らかの実数の値を出力させる問題です．
# 
...
# 用意したトレーニングデータセット（パラメータ更新にのみ用いるデータセット）の全部を人工知能が処理する時間の単位です．1 エポックだと全データを人工知能が舐めたことになります．2 エポックだと 2 回ほど人工知能が全データを舐めたことになります．学習の過程では繰り返しデータを人工知能に読ませるのです．
...
# 
# また，精度は英語では precision と記述されると思いますが，合っていますでしょうか．統計学においては精度（分散の逆数）の英訳は precision です．統計学と近い分野である機械学習の分野でもこれまでに伝統的な統計学の分野で利用されてきた「精度 = precision」の利用を否定しないのであれば，人工知能の評価指標としての presicion は positive predictive value（陽性適中率）のことを指していて，PPV と略記されます．精度 $v$ は以下の式で定義されます：
# 
...
# 機械学習モデルのパラメータの更新の仕方には種類があります．一括学習はバッチ学習とも呼ばれます．人工知能を成長させるときにデータを全部読み込ませた後に初めてパラメータを更新します．逐次学習はオンライン学習とも呼ばれます．人工知能を成長させるときにデータを1個ずつ読み込ませ，その都度パラメータを更新する方法です．ミニバッチ学習は全データからあらかじめ決めた分のデータを読み込ませ，その度にパラメータを更新する方法です．ミニバッチ学習のミニバッチサイズが全データサイズと等しいならそれは一括学習で，ミニバッチサイズが1なら逐次学習です．
...
# 
# このような入力と教師データのペアをいくつも用意し，それらを人工知能アルゴリズムに学習させます．教師あり学習の目的は，そのようにして構築した人工知能に新たな入力データが入力された場合に，そのラベルを予測させることです．
# 
# ここでは，ラベルを予測させる分類問題を例にしましたが，数値を予測させることも可能で，回帰問題にも利用可能です．教師あり学習法で成長させられる人工知能アルゴリズムとしては，ニューラルネットワーク，サポートベクトルマシン，線形回帰式，決定木をはじめとして様々なものがあります．
...
# 
# 教師なし学習法で人工知能アルゴリズムに学習させるデータは入力データのみです．教師なし学習法で成長させられる人工知能は与えられたデータに存在する傾向を把握するために用いられることが多いです．類似の性質を持つデータを集めるクラスタリングや，人の能力では理解できないような高次元に存在するデータを可視化することや，データの特徴の抽出等に利用されます．教師なし学習法のアルゴリズムとして用いられるものとしては，K-means，階層的クラスタリング法（単連結法をはじめとする階層的にデータを分類する手法の総称），主成分分析法，自己組織化マップ（ニューラルネットワークの一種）等の様々なものがあります．

# ### 強化学習法

# 強化学習法は教師あり学習法と完全に独立な学習法ではないと思います．正解の情報を利用するという点においては教師あり学習法と同様で，教師あり学習法の一種と考えることも可能なのではないでしょうか．しかし実際の強化学習法で行っていることは教師あり学習法で行っていることと様々な部分で異なっているように見え，現状の分類では強化学習法と教師あり学習法は別々に紹介される場合が多いです．
# 
# 強化学習法で用いる正解の情報は報酬（報酬を合計した価値も正解の情報）と呼ばれます．これに加えて強化学習法では，エージェントと呼ばれる何らかの行動をする存在を設定します．エージェントは何らかの環境に存在していて，一連の行動をすることによって得られる価値を最大化するように成長させられます．このときの価値は教師あり学習法で用いられるような正解データのような即時的に正解または不正解を判断できるようなものではなく，一連の行動の結果に対して与えられるものです．この点において，教師あり学習法と強化学習法は異なります．
# 
# 例えば，2人のキャラクター同士が何らかのステージで戦う格闘ゲームを例にすると，最大の報酬が与えられるのは敵であるキャラクターを倒したときであって，戦いの過程においてパンチを繰り出せとか，必殺技を放て等の個々の行動に対する正解は提示されません．
# 
# 強化学習法は現在最も活発に開発研究が行われている学習法であると言っても過言ではなく，日々様々な手法が考案されています．例えば，深層ニューラルネットワークを利用する深層強化学習アルゴリズムの深層 Q ネットワークは一部のゲームにおいては人以上の性能を示しています．強化学習法で利用されるアルゴリズムには，動的計画法，や Q 学習法等があります．深層学習法と相性が良く活発に研究されているものは Q 学習法です．
...
# 機械学習を利用した研究開発を行う際に最も大切なことは複雑なプログラムを作れるとかプログラミングを効率的にできるとか複雑な機械学習アルゴリズムを理解できるとかではありません．現在の機械学習の研究開発者にとって最も大切なことは人工知能を過学習させないこと，人工知能を正当に評価することです．

# ### 過学習

# 機械学習をしている最中において，人工知能は与えられたデータを繰り返し学習することでそのデータに自身を適合させます．そして，構築された人工知能の役割はその「与えられたデータ」以外の新たな別のデータに対して良い予測性能を発揮できることです．
# 
...
# ```{hint}
# 独立同分布の考え方です．調べてみましょう．
# ```

# 新たな人工知能を開発してそれを公開する場合，普通，その人工知能の性能をベンチマークし論文や学会で発表します．その性能を評価するために使うデータセットがテストセットです．もしこのテストセットが学習セットと同じような性質を持っているのであれば，その人工知能は新たなデータを処理するときに期待されるような性能を発揮できないかもしれません．
# 
# 学習セットとテストセットが独立でないのならば，学習セットに過剰適合させた人工知能では，あたかもとても良い性能をしているように見えてしまいます．人工知能の実際の現場での活用方法を鑑みるに，人工知能はある特定のデータにだけではなく，様々なデータに対応できる汎化能力を持たなければなりません．
