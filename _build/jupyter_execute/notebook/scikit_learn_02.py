...
# $
# 
# K-means ではこの値を最小化するようにクラスタへのインスタンスの割り当てを行います．つまり，K-means では以下の $E$ を最小化します．
# 
...
# ```{note}
# シルエットスコアはクラスタに所属するインスタンス間の平均距離（凝集度と言う）をそのクラスタから最も近いクラスタに存在しているインスタンスとの平均距離（乖離度と言う）から引いた値を凝集度または乖離度の内で大きな値で割った値を全 $k$ 個のクラスタについて計算して平均をとった値です．正三角形のそれぞれの頂点付近にインスタンスが散らばった本来は3個に分けるべきデータセットを3個に分けたときと2個に分けたときの数値を想像してみると解りやすいです．きれいに分離されている場合のシルエットスコアは1に近づきます．
# ```
