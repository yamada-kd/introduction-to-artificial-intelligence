...
# ネットワークを定義するクラスを以下のように書き換えました．`nn.Conv2d()` は畳み込み層のためのもの，`nn.MaxPool2d()` は最大値プーリングを計算するためのものです．また，`nn.Linear()` はこれまでにも紹介した通り全結合を計算するためのものです．チャネルというものは，例えば RGB のような色を表す情報です．CNN ではチャネルという単位に対して同様にフィルタ処理を行う，という計算をすることができます．また，カーネルサイズとありますが，このカーネルとはフィルタのことです．パディングというものは画像データを大きくするために，何らかの値（多くの場合0）で画像の外周を埋める作業です．畳み込み層やプーリング層によって画像が小さくなってしまいますが，これを補完するために行います．
# ```python
# class Network(nn.Module):
#     def __init__(self, OUTPUTSIZE):
#         super(Network, self).__init__()
