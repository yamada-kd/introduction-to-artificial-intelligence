...
# ```{note}
# 例えば，TD 誤差が正の値である場合，つまり，現在の Q 値は目標値より小さいので，この更新式によると Q 値は増加します．逆に，TD 誤差が負の場合は，現在の Q 値が目標値よりも大きいということなので，Q 値は減少します．
# ```
...
# 以上のようにグリッドワールドに関する問題を解くエージェントを構築できました．しかし，このエージェントはこのマップでしか答えを導き出せないでしょう．障害物の場所が少し変わるだけで機能しません．このマップに過学習しているということです．これに汎化性能を与えるためには様々な方法が考えられます．
# 
# 例えば，観測値を増やすことです．エージェントの左右上下のマップも情報も観測値として保有する方法が考えられます．そうすると，操作オブジェクトの上のマスに障害物がある場合はその方向には進まなくなるでしょう．
# 
# また，その上でデータ（この場合はマップ）をたくさん生成して異なる種類の障害物や広さを繰り返し学習させることでエージェントは様々なシナリオに対応する能力を養うことができると考えられます．
